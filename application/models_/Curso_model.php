...
class Curso_model extends CI_Model{
	...
	function buscar_curso($id){
		$this->db->select("c.*,cc.professor_id, pe.nome AS nome_coordenador")
		->from('curso c')
		->join('coordenador_curso cc','cc.curso_id=c.id')
		->join('professor p', 'p.id=cc.professor_id')
		->join('pessoa pe', 'pe.id=p.pessoa_id')
	...
	function deletar_curso($id){
	 ...
	 public function grade($curso_id)
     {
            ...
            {
             $sql="select d.periodo, d.titulo, 
                                  d.carga_horaria 
							from disciplina d
							join curso c on d.segmento_curso_curso_id = c.id 
                            where c.id=".$curso_id." and d.segmento_curso_segmento_id=".$segmento['id']." order by d.periodo";
              
					$segmento['horario']=$this->db->query($sql)->result();
            	$segmentos[]=$segmento;
				}
